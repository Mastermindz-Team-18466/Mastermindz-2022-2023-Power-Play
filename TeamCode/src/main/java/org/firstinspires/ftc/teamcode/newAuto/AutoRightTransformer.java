...
@Autonomous(name = "AutoRightTransformer", group = "Concept")
//@Disabled
public class AutoRightTransformer extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            if (currentTime - startTime >= 0 && cycles <= 4) {
...
//                                    .setTurnConstraint(Math.toRadians(330), Math.toRadians(270))
                ...
                if (currentTime - startTime >= 4100 && currentTime - startTime < 20500) {
            ...
            if (currentTime - startTime >= 20300 && moved) {
                telemetry.addLine("ran this part of the code!!");
                telemetry.update();
                endPosition = new Pose2d(endPosition.getX() - 5, endPosition.getY() + 23.5, Math.PI + Math.toRadians(17));
            ...
            if (currentTime - startTime >= 21700 && currentTime - startTime <= 24200) {
                inOutTake.setaVerticalPos(IntakeAndOuttake.verticalPos.GROUND);
                inOutTake.setaInstructions(IntakeAndOuttake.Instructions.TRANSFORMER_UP);
                inOutTake.setaSpecificInstruction(IntakeAndOuttake.specificInstructions.INTAKE_EXTENSION);
            }
            if (currentTime - startTime >= 22500 && moved2) {
                endPosition = new Pose2d(1.5 * 23.5 - 4, -3 * 23.5 + 56, Math.PI / 2 + Math.toRadians(85));
                drive.followTrajectorySequenceAsync(drive.trajectorySequenceBuilder(drive.getPoseEstimate())
                        .lineToSplineHeading(endPosition)
                        .build()
                );
            ...
            if (currentTime - startTime >= 24500 && !returned) {
                inOutTake.setaVerticalPos(IntakeAndOuttake.verticalPos.TOP);
                inOutTake.setaInstructions(IntakeAndOuttake.Instructions.STRAIGHT_DEPOSIT);
                inOutTake.setaSpecificInstruction(IntakeAndOuttake.specificInstructions.DEPOSIT_CONE);
                returned = true;
            }
            if (currentTime - startTime >= 27500 && park) {
                ...
                } else if (currentTime - startTime >= 28750 && park2) {
                    switch (position) {
                        case 2:
